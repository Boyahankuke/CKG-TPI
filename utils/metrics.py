...
def ndcg_at_k_batch(hits, k):
    ...
    """
    hits_k = hits[:, :k]
    # DCG是每个推荐项目的分数，除以它所在的位置，也就是说一个项目（item）推荐的排名越靠后，它折损的越严重。这里的顺序，是预测的顺序，而用到的分数，是数据集中的真实分数
...
def calc_metrics_at_k(cf_scores, train_user_dict, test_user_dict, user_ids, item_ids, Ks):
    ...
    for k in Ks:
        metrics_dict[k] = {}
        metrics_dict[k]['precision'] = precision_at_k_batch(binary_hit, k)  # 出现在前k个的数量除以k
        metrics_dict[k]['recall']    = recall_at_k_batch(binary_hit, k)  # 出现在前k个的数量除以测试集中该表位的所有的binding个数
