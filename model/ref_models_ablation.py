...
# Fold: 0 Epoch: 11 best auc: 0.7378482869959815 cur aupr: 0.398637684403767 cur auc: 0.7378482869959815
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def __init__(self):
        ...
        # densenet
        self.scale_down_ratio = 0.25
        self.growth_rate = 20
        self.transition_rate = 0.5
        self.num_dense_blocks = 4
        self.kernal_dense_size = 3
        self.n_layer = 2
        # encoder
        ...
        self.flatten_dim = 540

        # specialized embedding with positional one
        self.demb = Embeddings(21, self.emb_size, self.max_d, self.dropout_rate)
        self.pemb = Embeddings(21, self.emb_size, self.max_p, self.dropout_rate)
    ...
    def forward(self, d, p, d_mask, p_mask):
        d_emb = self.demb(d).mean(1)  # batch_size x seq_length x embed_size
        p_emb = self.pemb(p).mean(1)
        f = torch.cat((d_emb, p_emb), dim=1)
        score = self.decoder1(f)
        return score
